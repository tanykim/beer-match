define(['moment', 'textures'], function (moment, textures) {
    ...
    var updateGraph = function (option) {
        ...
        if (selected === 'matrix') {
            d3.selectAll('.js-matrix-block')
                ...
                .style('fill', function (d) { return d.fill[option]; })
    ...
    function createMatrixDataset (byHour, byDay, maxMatrix, legendCount) {
        ...
        var chromaId = function (d) {
            var id = 0;
            var step = maxMatrix / legendCount;
            for (var i = 1; i < legendCount + 1; i++) {
                if (d <= i * step) {
                    id = i - 1;
                    break;
                }
        ...
        return _.flatten(_.map(byHour, function (hour, i) {
            return _.map(hour.byDay, function (d, day) {
                return {
                    ...
                    fill: {
                        matrix: d === 0 ? '#FFF' : colors[chromaId(d)],
                        day: colors[0],
                        hour: colors[0]
                    },
                    val: d,
    ...
    function setTextures(svg, txColor) {
        $('#vis-matrix').find('def').remove();
        tx = textures.lines().size(6).lighter().background(txColor);
        svg.call(tx);
    }
    ...
    function drawBlocks(svg, dim, dataset, maxVal) {
        svg.selectAll('.js-matrix-block')
            ...
            .style('fill',function (d) { return d.fill.matrix; })
            ...
            .on('mouseout', function (d) {
                $('.js-matrix-tooltip').hide();
                d3.select(this).style('fill', d.fill[selected]);
            });
    }
    ...
    var drawMatrix = function (vis, byDay, byHour, maxVals, maxMatrix, legendCount, c) {
        ...
        //put legend
        E.drawChromaLegend(svg, dim.w, -40, legendCount, maxMatrix / legendCount, 'matrix', colors);
        ...
        //matrix dataset
        var dataset = createMatrixDataset(byHour, byDay, maxMatrix, legendCount);
        ...
        E.setTooltipText([maxBlock.str, maxBlock.val + ' check-ins'],
            ...
            maxBlock.y.matrix);
        setTextures(svg, colors[colors.length-1]);
        mb = maxBlock.order;
