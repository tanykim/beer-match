...
// Check the original library at https://www.npmjs.org/package/node-untappd
// ********
//
...
var UntappdClient = function(debug) {
	...
	var req = function(method, path, params, data, callback) {
		...
		var request = HTTPS.request(options,function(response){
			...
			response.on('data',function(incoming){
				if (debug) {
					console.log('data: ', incoming.length);
				}
				data += incoming;
			})
			response.on('end',function(incoming){
				if (debug) {
					console.log('end: ', incoming?incoming.length:0);
				}
	...
	that.verify = function(callback) {
		if (callback === undefined || callback === null) {
			throw new Error('callback cannot be undefined or null.');
		}
	...
	that.userFeed = function(callback,lookupUser,limit,max_id) {
		if (lookupUser === undefined || lookupUser === null) {
			throw new Error('lookupUser cannot be undefined or null.');
		}
		if (callback === undefined || callback === null) {
			throw new Error('callback cannot be undefined or null.');
		}
