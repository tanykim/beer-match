...
var timeConvert = {
    ...
    range: function(t, timezone) {
        return [moment(t).format('YYYYMMDD'), moment().tz(timezone).format('YYYYMMDD')];
    },
    dayId: function(t) {
...
function getCountByPeriod(checkins, timezone, timeRangeStr) {
    ...
    var unitCounts = [
        ...
        (timeRange[0].day() > timeRange[1].day() ? 1 : 0),
        Math.ceil(timeRange[1].diff(timeRange[0], 'months', true)) +
        (timeRange[0].date() > timeRange[1].date() ? 1 : 0)
    ];
...
var User = function (userinfo, timezone, checkins) {
    ...
    this.scoreCount = _.pick(_.countBy(_.pluck(checkins, 'rating_score'), function (d) {
        return d;
    ...
    var period = moment(tRange[1], 'YYYYMMDD').diff(moment(tRange[0], 'YYYYMMDD'), 'month') > 3 ?
        'month' : 'week';
