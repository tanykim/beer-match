...
var timeConvert = {
    timezoned: function (t, timezone) {
        ...
        return moment.tz(momentT, timezone);
    },
    range: function(t, timezone) {
        return [moment(t).format('YYYYMMDD'),
            moment().tz(timezone).format('YYYYMMDD')];
            // '20141105'];
    },
    dayId: function(t, timezone) {
        var current = moment().tz(timezone).startOf('day');
        // var current = moment('20141105', 'YYYYMMDD');
        var momentT = moment(t, 'ddd, DD MMM YYYY HH:mm:ssZ');
        var tzd = moment.tz(momentT, timezone);
        return _.object(_.map(['day', 'week', 'month'], function (d) {

        console.log(tzd.format('YYYYMMDD hh:mm:ss Z'),
            current.clone().add(1, d + 's').startOf(d).diff(tzd, d + 's')
            );


            return [d, current.clone().add(1, d + 's').startOf(d)
                .diff(tzd, d + 's')];
        }));
        }
};

function getCountByPeriod(checkins, timezone, timeRangeStr) {

    var tRange = _.map(timeRangeStr, function (d) {
    ...
    function getByUnit(unit) {
        return _.countBy(_.pluck(checkins, 'created_at'), function (d) {
            ...
            var dayId = timeConvert.dayId(d, timezone);
            return dayId[unit];
        });
    }
    ...
    var daysCount = Math.ceil(tRange[1].diff(tRange[0], 'days', true)) + 1;

    //fill 0 if no check in
    var byDayFull = _.map(_.range(daysCount), function (i) {

        var date = tRange[0].clone().add(i, 'days');
        // var timezoned = timeConvert.timezoned(date, timezone);
        var dateId = timeConvert.dayId(date, timezone);
        ...
        if (!_.isUndefined(byDay[dateId.day])) {
            day = byDay[dateId.day];
        }
    ...
    var unitCounts = [
        daysCount,
        Math.ceil(tRange[1].diff(tRange[0], 'weeks', true)) +
        (tRange[0].day() > tRange[1].day() ? 1 : 0),
        Math.ceil(tRange[1].diff(tRange[0], 'months', true)) +
        (tRange[0].date() > tRange[1].date() ? 1 : 0)
    ];
...
function getVenueByTime(checkins, period, timezone) {
    ...
    return _.sortBy(_.map(_.omit(_.groupBy(_.map(checkins, function (d) {
        ...
        var dayId = timeConvert.dayId(d.created_at, timezone);
...
var User = function (userinfo, timezone, checkins) {
    ...
    var since = timeConvert.timezoned(userinfo.since, timezone);
    ...
    var tRange = timeConvert.range(since, timezone);
    ...
    var period = moment(tRange[1], 'YYYYMMDD')
        .diff(moment(tRange[0], 'YYYYMMDD'), 'month') > 3 ?
        'month' :
        'week';
    this.venueByTime = getVenueByTime(checkins, period, timezone);
